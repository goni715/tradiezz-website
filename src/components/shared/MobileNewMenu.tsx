...
const MobileNewMenu = ({ setIsMenuOpen, userInfo, onLogout }: TProps) => {
  ...
  const MenuLink = ({ path, label, icon: Icon }: { path: string; label: string; icon: React.ReactNode }) => (
    <button
      ...
      <div className="h-full overflow-y-auto flex flex-col">
        ...
        <div className="sticky top-0 bg-background border-b border-border p-4 space-y-4">
          {userInfo?.userId && (
            <div
              onClick={() => {
                handleNavigate("/dashboard/candidate/profile");
              }}
            ...
            >
              <div className="relative w-10 h-10 rounded-full overflow-hidden shrink-0 border border-border">
                <Image
                  src={userInfo?.profileImg || "/images/profile_placeholder.png"}
                  onError={(e) => {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="space-y-1">
            ...
            <MenuLink path="/" label="Home" icon={<Home className="w-5 h-5" />} />
            <MenuLink path="/find-work" label="Find Work" icon={<Briefcase className="w-5 h-5" />} />
            {userInfo?.role === "employer" && (
              <MenuLink path="/find-candidates" label="Find Talent" icon={<Users className="w-5 h-5" />} />
            )}
            <MenuLink path="/blogs" label="Resources" icon={<FileText className="w-5 h-5" />} />
            <MenuLink path="/contact" label="Contact" icon={<Mail className="w-5 h-5" />} />
          </div>
            ...
            <div className="space-y-1">
              ...
              {userInfo?.role === "employer" ? (
                ...
                <>
                  ...
                  />
                  <MenuLink
                    path="/dashboard/candidate/my-jobs"
                    label="Saved Jobs"
                    icon={<BriefcaseBusiness className="w-5 h-5" />}
                  />
                  <MenuLink
        ...
        <div className="sticky bottom-0 border-t border-border p-4 space-y-2 bg-background">
          {userInfo?.userId ? (
            ...
            <button
              ...
              className="w-full px-4 py-3 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors font-medium text-sm"
            >
              Sign In
            </button>
          )}
